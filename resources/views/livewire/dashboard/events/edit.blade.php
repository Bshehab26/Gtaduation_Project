...
<div>
        ...
        <div class="col-10">
            <select wire:model='form.organizer_id' name="organizer" class="form-select" aria-label="multiple select example">
                @foreach ($organizers as $organizer)
                    <option value="{{ $organizer->id }}" {{ $organizer->id === $form->organizer_id ? 'selected' : '' }}>{{ $organizer->first_name }} {{ $organizer->last_name }}</option>
                @endforeach
            </select>
            ...
            <div class="py-2 px-4 rounded col-9">
                ...
                <div class="dropdown">
                    ...
                    @foreach ($currentCategory->subcategories as $sub)
                        @php
                            if ($event->subcategories->contains($sub->id)){
                                continue;
                            }
                        @endphp
                        <div wire:key='{{ $sub->id }}'>
    ...
    {{-- END CATEGORY INFORMATION SECIOTN --}}
